...
if __name__ == '__main__':
    ...
    json_dir_in = "outputs_intermediate/retriever_output"
    question_classification_json_dir_in = "outputs_intermediate/question_classification_output"
    json_dir_out = "outputs_intermediate/retriever+QAClass_combined_output"
    ...
    sep = "*" * 20
    print(sep)
    print("Combining Retriever results")
    print(sep)
    
    # mode_names = ["train", "test", "dev"]
    mode_names = ["Pipeline"]
    filename = "MT2NET_input"
    # mode_names = ["test"]
    for mode in mode_names:
        json_in = os.path.join(json_dir_in, f"{filename}.json")
        question_classification_json_in = os.path.join(question_classification_json_dir_in, f"{filename}.json")
        json_out_inference = os.path.join(json_dir_out, "reasoning_stage_input.json")
        ...
        print(f"Convert {filename} set done")
