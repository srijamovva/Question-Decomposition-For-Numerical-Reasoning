...
def combine_predictions(span_selection_json_in, program_generation_json_in, test_file_json_in, output_dir):
    ...
    print(len(span_selection_data))
    print(len(program_generation_data))
    print(len(orig_data))
...
def evaluation_prediction_result(span_selection_json_in, program_generation_json_in, test_file_json_in, output_dir):
    ...
    print("Generated the combined Span Selection + Program Generation file.")

    orig_data = json.load(open(test_file_json_in))

    num_examples = len(orig_data)

    if "answer" in orig_data[0]['qa'].keys():
        ...
        for example in orig_data:
            ...
            # both span selection
                ...
                exact_acc, f1_acc = get_span_selection_metrics(str(pred["predicted_ans"]), gold_ans)
...
if __name__ == '__main__':
    ...
    test_file_json_in = os.path.join("input_data", f"MT2NET_input.json")
